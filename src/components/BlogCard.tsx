...
export default function BlogCard({ blog }: { blog: BlogCardData }) {
  ...
  return (
    ...
    >
      <div className="h-32 sm:h-28 relative overflow-hidden bg-gradient-to-br from-ink-50 to-ink-100">
        {blog.featuredImage ? (
          <Image
      ...
      <div className="p-4 sm:p-5">
        <h3 className="font-voice font-semibold text-base sm:text-lg mb-2 leading-snug group-hover:text-accent-600 transition-colors line-clamp-2">
          {blog.title}
        </h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-ink-50">
          <div className="flex items-center gap-2 min-w-0">
            <div
              className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-semibold text-white shadow-sm shrink-0"
              style={{ background: colors.text }}
            >
              {blog.author.name.charAt(0).toUpperCase()}
            </div>
            <span className="text-xs text-ink-400 font-medium truncate">{blog.author.name}</span>
          </div>
          <span className="text-xs text-accent-600 font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 group-hover:translate-x-1 shrink-0">
            Read →
          </span>
        </div>
      </div>
